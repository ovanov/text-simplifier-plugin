...
function authPromptMessage() {
    return "Bitte erneut anmelden (Klicke auf das EinfachLesen-Symbol)";
}
function displayResult(targetElement, simplifiedText, originalText) {
    ...
    resimplifyBtn.onclick = async () => {
        ...
        if (!result.ok) {
            const msg = result.status === 401 ? authPromptMessage() : result.detail || "Fehler";
